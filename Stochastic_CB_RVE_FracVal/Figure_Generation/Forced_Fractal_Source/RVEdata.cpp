...
void RVEdata::Build_RVE()
{
    ...
    std::cout << "Shell/Potential Maps Finished" << std::endl;
    ...
    for (int i = 0; i <= Xbound; i++)
    {
        for (int k = 0; k <= Zbound; k++)
        {
            for (int j = 0; j <= Ybound; j++)
            {
                ...
                {
                    //check << i << " " << j << " " << k << " " << 0.1 << " 100 0 " << std::endl;
                    num_pot++;
                }
                if (Shell[counter] == 1)
                {
    ...
    check << Total_Num_Particles +  num_pot << std::endl << std::endl;
    for (int i = 0; i < Total_Num_Particles; i++)
    {
        check << GetCBParticle(i)->GetXCoordinate() << " " << GetCBParticle(i)->GetYCoordinate() << " " << GetCBParticle(i)->GetZCoordinate() << " " << 5 <<  " 0 0 0" << std::endl;
    }
    counter = 0;
    for (int i = 0; i <= Xbound; i++)
    {
        for (int k = 0; k <= Zbound; k++)
        {
            for (int j = 0; j <= Ybound; j++)
            {
                if (Potential[counter] == 1)
                {
                    check << i << " " << j << " " << k << " " << 0.25 << " 0 100 0" << std::endl;
                }
                if (Shell[counter] == 1)
                {
                    //check << i << " " << j << " " << k << " " << 0 << " 100 0 0" << std::endl;
                }
                counter++;
            }
        }
    ...
    //Output potential and shell for checking. Files get really 'uge really quicklike
    std::ofstream check2;
    check2.open("check2.csv");
    check2 << Total_Num_Particles << std::endl << std::endl;
    for (int i = 0; i < Total_Num_Particles; i++)
    {
        check2 << GetCBParticle(i)->GetXCoordinate() << " " << GetCBParticle(i)->GetYCoordinate() << " " << GetCBParticle(i)->GetZCoordinate() << " " << 5 <<  " 0 0 0" << std::endl;
    }
    check2.close();

    /*
    //Output coordinates for checking and generating images
    std::ofstream check;
...
void RVEdata::Place_Forced_Agglomerates(long int num_rand)
{
   ...
   // Agg size = 100 particles num_forced_agg = floor(((Total_Num_Particles - (floor((Total_Num_Particles*Pct_Rand)/100.)*100.))*(1.-Pct_Fill))/100.);
    std::cout << "num_forced_agg: " << num_forced_agg << std::endl;
        ...
        while (Good == false)
        {
            ...
            if (Got_Neighbors == true && Good == true)
            {
                ...
                for (int z = 0; z < Agglomerates_Indices[next_agglomerate].size(); z++)
                {
                    ...
                    long int test_index = next_index + Agglomerates_Indices[next_agglomerate][z];
                    //Map the particle
                    Map_CB(running_index);
                    
                    int xt, yt, zt;
                    xt = floor(test_index/((Ybound+1)*(Zbound+1)));
                    zt = floor((test_index-(xt*(Ybound+1)*(Zbound+1)))/(Ybound+1));
                    yt = test_index - (xt*(Ybound+1)*(Zbound+1)) - (zt*(Ybound+1));
                    //std::cout << " xyz forced (" << running_index << ") = " << xt << "," << yt << "," << zt << "," << std::endl;
                    //Increment running index
                    running_index += 1;
                }
            }
...
void RVEdata::Initialize_CB_Potential_Shell_Maps()
{
    ...
    for (int x = 0; x < NumCBFillers; x++)
    {
        for (int y = 0; y <= GetCBMatType(x)->GetMaxDiameter() - GetCBMatType(x)->GetMinDiameter(); y++)
        {
            ...
            {
                output << x0+CB_Potential_Maps_X[Map_Count][p] << " " << y0+CB_Potential_Maps_Y[Map_Count][p] << " " << z0+CB_Potential_Maps_Z[Map_Count][p] << " " << 100 << " " << 0 << " " << CB_Potential_Maps_Indices[Map_Count][p] + zer <<std::endl;
            }
            
            for (int p = 0; p < CB_Shell_Maps_Indices[Map_Count].size(); p++)
            {
                output << x0+CB_Shell_Maps_X[Map_Count][p] << " " << y0+CB_Shell_Maps_Y[Map_Count][p] << " " << z0+CB_Shell_Maps_Z[Map_Count][p] << " " << 0 << " " << 100 << " " << CB_Shell_Maps_Indices[Map_Count][p] << std::endl;
            }
            
            output.close();
            */
            std::ofstream out;
            out.open("check_map_" + std::to_string(Map_Count) + ".csv");
            out << CB_Potential_Maps_Indices[Map_Count].size() + CB_Shell_Maps_Indices[Map_Count].size() + 1 << "\n\n";// + CB_Shell_Maps_Indices[Map_Count].size() << std::endl << std::endl;
            long int counter_p = 0;
            long int counter_s = 0;
            ...
            for (int i = 0; i <=Xbound; i++)
            {
                for(int k = 0; k <= Zbound; k++)
                {
                    for (int j = 0; j <= Ybound; j++)
                    {
                        if (CB_Potential_Maps_Indices[Map_Count][counter_p] + zer == p)
                        {
                            //out << i << " " << j << " " << k << " " << 0.5 << " " << 100 << " " << 0 << "," << p << std::endl;
                            out << i << " " << j << " " << k << " 0.2 100 0 0" << std::endl;

                            counter_p++;
                        }
                        
                        if (CB_Shell_Maps_Indices[Map_Count][counter_s] + zer == p)
                        {
                            //out << i << " " << j << " " << k << " " << 0.5 << " " << 0 << " " << 100 << "," << p << std::endl;
                            out << i << " " << j << " " << k << " 0.2 0 100 0 " << std::endl;
                            counter_s++;
                        }
...
void RVEdata::SolveForConductivity()
{
    ...
    std::cout << "Cond = " << RVEconductivity << std::endl;
}
...
{
    fout << RVEresistance << "," << RVEconductivity << ",";
}
...
{
    //Reset all the storage thingy dingys
    Shell.reset();
    Potential.reset();
...
void RVEdata::Output_Ovito_File(std::ofstream &fplot, std::string uid, std::string bashid)
{
    std::string current_dir = getcwd(NULL,0);
    std::string directory("/Training_Data/" + std::to_string(Xbound*mult) + "_" + std::to_string(Wcb));
