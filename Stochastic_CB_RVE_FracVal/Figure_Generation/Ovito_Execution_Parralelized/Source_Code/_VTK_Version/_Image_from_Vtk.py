...
def generate_image(XY,file,directory,folder_name, a, b):
    print("Opening " + file)
    OpenDatabase(directory + "/" + file + ".xyz", 0)
    ...
    MoleculeAtts.radiusVariable = "var0"
    ...
    AnnotationAtts.axes3D.bboxLocation = (0, XY, 0, XY, 0, XY)
    ...
    View3DAtts.focus = (int(XY/2), int(XY/2), 0)
...
for names in os.listdir(directory):
    if names[0] == "_":
        Images_Folders.append("/" + names)
        continue

# Get user input for pixel count required..
pixel = str(sys.argv[3]) #str(input())
a, b = pixel.split('x')
folder_name = "/_" + sys.argv[1] + "_" + a + "x" + b + "_images"
XY, ignore = str(sys.argv[1]).split('_')
path = directory + folder_name
filename = sys.argv[2]
base, extension = os.path.splitext(filename)
XYZ_file_base, XYZ_ext = os.path.splitext(base)
...
image_file = XYZ_file_base + ".png"
if image_file in Image_Files:
    pass
else:
    os.system('xz -d -k ' + directory + "/" + XYZ_file_base + '.xyz.xz')
    pass_fail = generate_image(int(XY), XYZ_file_base, directory, folder_name, a, b)
    if pass_fail == 0:
        os.system('rm '+ directory + "/" + XYZ_file_base + '.xyz')
        #os.system('rm '+ directory + "/" + XYZ_file_base + '.xyz.xz')
    else:
        os.system('rm '+ directory + "/" + XYZ_file_base + '.xyz')
